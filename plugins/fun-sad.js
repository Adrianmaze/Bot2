...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (m.mentionedJid.length > 0) {
        ...
        str = `\`${name2}\` está triste. 😥`.trim(); // En caso de no mencionar ni citar a nadie
    }

    if (m.isGroup) {
        // Videos para la reacción triste
        ...
        // Elegimos un video al azar
        const videos = [pp1, pp2, pp3, pp4, pp5, pp6, pp7, pp8];
        const video = videos[Math.floor(Math.random() * videos.length)];

        // Enviamos el mensaje con video y menciones
        await conn.sendMessage(m.chat, {
            video: { url: video },
            gifPlayback: true,
            caption: str,
            mentions: [who] // Mencionamos a la persona destinataria
        }, { quoted: m });
    }
};
